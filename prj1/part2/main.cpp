...
/* for snapshot */
bool im_initiator = false;
int initiator = ID_UNKNOWN;
...
void parse_command(int argc, char **argv) {
  string usage = "-h <hostfile> -c <count> [-s <snapshot_count> -d <drop rate> -t <delay> -o <timeout> -l <try limit> -v]";
  ...
  char opt;
  while ((opt = getopt(argc, argv, "s:h:c:d:t:o:l:v")) != -1) {
    if (opt == 'h') {  // -h <hostfile>
...
void parse_message(unsigned char *buf) {
  ...
  case DATA_MSG_TYPE: {
    ...
    if (proposed_seqs.find(key) == proposed_seqs.end()) {
      /* record channel */
      add_record(buf, DATA_MSG_SIZE * sizeof(uint32_t), s.sender);
  ...
  case ACK_MSG_TYPE: {
    ...
    if (s.sender == my_id and (not messages_acked[s.msg_id][s.proposer])) {
      /* record channel */
      add_record(buf, ACK_MSG_SIZE * sizeof(uint32_t), s.proposer);
  ...
  case SEQ_MSG_TYPE: {
    ...
    if (seq_messages_recved.find(key) == seq_messages_recved.end()) {
      /* record channel */
      add_record(buf, SEQ_MSG_SIZE * sizeof(uint32_t), s.sender);
  ...
  case ACK_SEQ_MSG_TYPE: {
    ...
    if (s.sender == my_id and (not seq_messages_acked[s.msg_id][s.receiver])) {
      /* record channel */
      add_record(buf, ACK_SEQ_MSG_SIZE * sizeof(uint32_t), s.receiver);
...
void message_manager(int message_id) {
  ...
  for (int i = 0; i < try_limit; ++i) {
	...
	int send_len = pack_data(buf, my_id, message_id, 0);
	add_send_message(buf, send_len, id);
      }
  ...
  /* after finishing this message, start snapshot */
  if (im_initiator and (int)message_id == snapshot_count - 1)
      start_snapshot();
...
int main(int argc, char **argv) {
  ...
  /* initialize tcp listeners and senders */
  init_snapshot();

  /* start snapshot immediately if snapshot_count is 0 */
  if (im_initiator and snapshot_count == 0)
    start_snapshot();
