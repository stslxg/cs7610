...
static const char *TCP_PORT = "1154";
...
/* the following structs are for the snapshot */
typedef struct {
  uint32_t type; // must be equal to 6
  uint32_t initiator; // the initiator of the snapshot
  uint32_t sender; // the sender of the Marker
} MarkerMessage;

typedef struct {
  uint32_t type; // must be equal to 7
  uint32_t channel; // the incoming channel id of the following message, use MAX_HOST to denote the end
} SnapshotMessage;
...
#define MARKER_MSG_TYPE 6
#define SNAPSHOT_MSG_TYPE 7
...
#define MARKER_MSG_SIZE 3
#define SNAPSHOT_MSG_SIZE 2

#define ID_UNKNOWN (-1)

/* status of snapshot */
#define STATUS_WAIT 3
#define STATUS_RECORD 4
#define STATUS_SEND 5
#define STATUS_FINISH 6
#define STATUS_PRINT 7
